        ...
        <div className="min-w-0 flex-1">
          ...
          {expanded ? (
            <ul className="mt-3 flex max-h-56 flex-col gap-2 overflow-auto">
              {operation.error ? <li className="text-xs text-zinc-700 dark:text-zinc-300">{operation.error}</li> : null}
              {operation.failures.map((failure, index) => (
                <li key={index} className="rounded-md bg-zinc-50 p-2 text-xs dark:bg-zinc-950">
                  ...
                  <span className="mt-0.5 block font-mono text-[11px] whitespace-pre-wrap text-zinc-700 dark:text-zinc-300">{failure.message}</span>
                </li>
              ))}
            </ul>
          ) : null}
          <div className="mt-2 flex gap-3">
            {operation.failures.length || operation.error ? (
              <button
                ...
                className="text-xs font-medium text-zinc-700 underline decoration-zinc-300 underline-offset-4 hover:decoration-zinc-600 focus-visible:ring-2 focus-visible:ring-zinc-500 focus-visible:outline-none dark:text-zinc-300"
              >
